...
public class primAlgorithm {
    ...
    public static void main(String[] args) {
        ...
        //邻接矩阵的关系使用二维数组表示.使用10000这个数表示两个点不连通
        int[][] weight = new int[][]{
...
class MinTree {
    ...
    public void prim(MGraph graph, int v) {
        ...
        for (int k = 1; k < graph.vertex; k++) {//因为有vertex个顶点，prim算法结束后由graph.vertex-1 边
            //这个是确定每一次生成的子图，和哪个节点的距离最近
            for (int i = 0; i < graph.vertex; i++) {//i 节点表示被访问过的节点
                for (int j = 0; j < graph.vertex; j++) { //j节点表示还没有被访问过的节点
